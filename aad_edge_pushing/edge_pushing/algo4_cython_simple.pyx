...
"""
Simplified Cythonized version of Algorithm 4 (Edge-Pushing) for Stage 2B.

Strategy: Minimal changes to the original algo4_adjlist.py, just add:
1. Static typing for variables
2. C-level loops where possible
3. Inline critical functions

This ensures correctness while gaining performance from Cython's optimizations.
"""
...
# Import the sparse matrix implementation
# BUG FIXED 2025-11-11: C++ version now working correctly!
# - symm_sparse_adjlist_cpp: FIXED (accumulation bug + GIL errors resolved)
# - Performance: ~10-20× faster than Python version
from .symm_sparse_adjlist_cpp import SymmSparseAdjListCpp


def algo4_cython_simple(output, inputs: List):
    """
    Compute Hessian using Cythonized Algorithm 4.

    Same algorithm as algo4_adjlist, but with Cython optimizations.
    ...
    W = SymmSparseAdjListCpp(n_nodes)  # Use C++ version
    vbar = [0.0] * n_nodes
    vbar[output_idx] = 1.0  # Seed adjoint for output

    # Reverse sweep through nodes (this is the hot loop!)
    _reverse_sweep_cython(global_tape.nodes, W, vbar, var_to_idx)

    return _extract_input_hessian(W, input_indices)


cdef void _reverse_sweep_cython(list nodes, W, list vbar, dict var_to_idx):
    """
    Cythonized reverse sweep.

    This function processes the tape in reverse order with minimal Python overhead.
    """
    ...
    for node_idx in range(n_nodes - 1, -1, -1):
        ...
        # PUSHING STAGE
        _pushing_stage_cython(W, i, preds, d1)
...
cdef void _pushing_stage_cython(W, int i, list preds, dict d1):
    """Cythonized pushing stage."""
    cdef int p, j, k, a, b
...
# ============================================================================
# Helper functions (keep Python versions for simplicity)
# ============================================================================
