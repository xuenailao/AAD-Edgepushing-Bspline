"""
Algorithm 4 (Edge-Pushing) with C++ STL optimized adjacency list.

This version uses SymmSparseAdjListCpp which has:
- C++ std::unordered_map for storage (no Python object overhead)
- C++ std::unordered_set for adjacency lists
- nogil support for future parallelization
- Expected 2-3× additional speedup over Cython V2
"""
...
# Try to import C++ version, fallback to V2 if not available
try:
    from .symm_sparse_adjlist_cpp import SymmSparseAdjListCpp as SymmSparseAdjList
    print("Using C++ STL optimized version")
except ImportError:
    print("Warning: C++ version not available, falling back to V2")
    try:
        from .symm_sparse_adjlist_v2 import SymmSparseAdjListV2 as SymmSparseAdjList
    except ImportError:
        from .symm_sparse_adjlist import SymmSparseAdjList


def algo4_adjlist_cpp(output: ADVar, inputs: List[ADVar]) -> np.ndarray:
    """
    Compute Hessian using Algorithm 4 with C++ STL optimization.

    This version uses SymmSparseAdjListCpp which has:
    - std::unordered_map<int64, double> for storage
    - std::unordered_set<int> for adjacency
    - Complete removal of Python object overhead
    - nogil support for future parallelization
    ...
    W = SymmSparseAdjList(n_nodes)  # ← Uses C++ STL optimized version!
    vbar = [0.0] * n_nodes
    ...
    for node in reversed(global_tape.nodes):
        ...
        # PUSHING STAGE: Edge-pushing (OPTIMIZED with C++ STL!)
        _pushing_stage_adjlist(W, i, preds, d1)
...
def _pushing_stage_adjlist(W: SymmSparseAdjList, i: int, preds: List[int],
                            d1: Dict[int, float]) -> None:
    """
    OPTIMIZED pushing stage using C++ STL adjacency list.

    KEY OPTIMIZATION:
    - O(degree(i)) neighbor lookup instead of O(n)
    - C++ STL containers (no Python overhead)
    - Ready for nogil parallelization

    Args:
        W: C++ STL optimized sparse matrix
        i: Current node index
        preds: List of predecessor indices (deduplicated)
        d1: First-order derivatives ∂φᵢ/∂xⱼ
    """
    # ✨ O(degree(i)) with C++ STL optimization
...
def _creating_stage(W: SymmSparseAdjList, preds: List[int],
...
# ============================================================================
# Shared helper functions (same as algo4_adjlist.py)
# ============================================================================
...
def _extract_input_hessian(W: SymmSparseAdjList, input_indices: List[int]) -> np.ndarray:
