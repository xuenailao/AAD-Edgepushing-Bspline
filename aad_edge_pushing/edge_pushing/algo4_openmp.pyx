...
"""
OpenMP parallelized version of Algorithm 4 (Stage 2B Phase 2).

Key features:
1. Uses Cython's prange for parallel loops
2. Thread-safe matrix updates with atomic operations
3. Optimized for 128-core systems
4. nogil sections for true parallelism
"""
...
cdef void _reverse_sweep_openmp(list nodes, W, list vbar, dict var_to_idx, int n_threads):
    ...
    but inner loops (over neighbors) can be parallelized.
    """
...
cdef void _pushing_stage_openmp(W, int i, list preds, dict d1, int n_threads):
    ...
    Uses thread-safe atomic updates to W matrix.
    """
    ...
    # Each thread processes a subset of neighbors independently
    for idx in range(n_neighbors):
        p, w_pi = neighbors[idx]

        if p == i:
            # Case 1: Diagonal W(i,i) contribution
            # Nested parallel loop over predecessor pairs
...
cdef void _creating_stage_openmp(W, list preds, dict d2, double vbar, int n_threads):
    ...
    cdef list d2_items = list(d2.items())
    cdef int n_items = len(d2_items)
    cdef int idx
